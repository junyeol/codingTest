...
public class Controller {
	...
	public void start() {
		...
		while(true) {
			...
			//3. 로그아웃 시작
				...
				ArrayList<ProductDTO> wishList = isSelectAll(this.condition_Cart); // 장바구니를 새로운 배열에 등록	
				boolean flag = false;
			...
			//4. 상품 목록 출력 시작
				...
				ArrayList<ProductDTO> productList = isSelectAll(this.condition_Product);
			...
			//5. 장바구니에 상품담기 시작
				...
				ArrayList<ProductDTO> productList = isSelectAll(this.condition_Product);//상품 목록 전체를 받아와서
			...
			//6. 장바구니에 담긴 상품 목록 출력 시작
				...
				ArrayList<ProductDTO> wishList = isSelectAll(this.condition_Cart);
			...
			//7. 장바구니에 담긴 상품 구매 시작
				...
				ArrayList<ProductDTO> wishList = isSelectAll(this.condition_Cart); // 장바구니 내역 불러오기
				if(wishList.size() <= 0) {
					continue;
				}

				ArrayList<ProductDTO> productList = isSelectAll(this.condition_Product); // 상품 내역 불러오기
				...
				for(ProductDTO data: hashList) {
					...
					if(!wishflag) {
						this.loginView.removeFail(); //this.loginView.장바구니 정리가 안되었습니다. 관리자에게 문의해주세요();
					}
				}
			...
			//8. 장바구니에서 상품 빼기 시작
				...
				ArrayList<ProductDTO> datas = isSelectAll(this.condition_Cart); // isCart()에서 데이터 받아옴
				if(datas.size() <= 0) {
					continue;
				}
	...
	public ArrayList<ProductDTO> isSelectAll(String condition_name) {
		ProductDTO condition = new ProductDTO();
		condition.setCondition(condition_name);
